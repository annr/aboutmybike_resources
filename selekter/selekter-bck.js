...
;(function ( $, window, document, undefined ) {
  ...
  function Plugin( element, options ) {
    ...
    defaults.requiredFields.forEach(function(field) {
      if($el.find('input[name='+field+']').length === 0) {
  ...
  Plugin.prototype = {
    ...
    init: function() {
      ...
      $(this.element).find('input[name='+this.options.requiredFields[0]+']').val(this.options.scale);
    ...
    updatePositionValues: function(el, fieldsArray, values) {
      console.log('updating position values...');
      $(el).find('input[name='+fieldsArray[1]+']').val(values.xValue);
      $(el).find('input[name='+fieldsArray[2]+']').val(values.yValue);
    },
          ...
          $('.draggable').offset({
            ...
            left: left
              ...
              opt.onSelectMove(opt.form, opt.requiredFields, {
                xValue: Math.round(top - $(opt.container).offset().top),
                yValue: Math.round(left - $(opt.container).offset().left),
              });
          });
        });
